...
import { listDispatchEligibleDrivers } from "@/lib/api/drivers";
...
export default function TripsPage() {
  ...
  const createAndDispatchTrip = async () => {
    ...
    if (selectedVehicle && cargoWeight > selectedVehicle.maxLoadCapacity) {
      setFormError(
        "Cargo weight must not exceed the selected vehicle capacity.",
      );
      return;
    }
          ...
          <div className="mt-4 grid gap-3">
            ...
            <Card className="border border-sky-500/30 bg-sky-500/10 p-3">
              <p className="text-xs font-medium text-sky-500">
                Capacity check: cargo weight must not exceed the selected
                vehicle capacity.
              </p>
            </Card>
            <Button

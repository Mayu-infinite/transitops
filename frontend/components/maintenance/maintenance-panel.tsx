...
export function MaintenancePanel() {
  ...
  const { data, loading, error, reload } = useApiData<MaintenanceData>(
    async () => {
  ...
  const vehicleName = useMemo(() => {
    const map = new Map(
      (data?.vehicles ?? []).map((v) => [v.id, v.registrationNumber]),
    );
  ...
  const columns: Column<MaintenanceRecord>[] = [
    {
      ...
      header: "Vehicle",
      render: (r) => (
        <span className="font-medium">{vehicleName(r.vehicleId)}</span>
      ),
    },
    {
      key: "description",
    ...
    { key: "startDate", header: "Date", render: (r) => fmtDate(r.startDate) },
    {
      ...
      render: (r) => (
        <StatusBadge
          tone={MAINTENANCE_STATUS_TONE[r.status]}
      ...
      <Card className="h-fit border border-border/80 bg-surface/95 p-5 shadow-sm shadow-black/5">
        ...
        <div className="mt-5 flex gap-3 rounded-lg border border-amber-500/20 bg-amber-500/10 p-3">
          ...
          <p className="text-xs leading-relaxed text-muted">
            A vehicle with an{" "}
            <span className="font-medium text-foreground">active</span> service
            record (Open / In Progress) is marked{" "}
            <span className="font-medium text-foreground">In Shop</span> and
